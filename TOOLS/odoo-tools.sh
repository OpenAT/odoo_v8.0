...
if [ "$SCRIPT_MODE" = "updatetranslation" ]; then
    ...
    su - postgres -c "psql -A -t -q -c -d o8_intdadi_dev1 -t -c \"SELECT name FROM ir_module_module WHERE state = 'installed'\"">${INSTANCE_PATH}/${DBNAME}/INSTALLEDMODULES # this will work
